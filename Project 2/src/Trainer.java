...
/**
 * Trainer is an instance of Trainer, subclass of Entity
 *
...
public class Trainer extends Entity {
    ...
    public void usePotion(int pokeIndex) {
        if (this.hasPotion()) {
            Pokemon p = this.pokemon.get(pokeIndex);
            p.heal();
            this.potions -= 1;
            this.pokemon.set(pokeIndex, PokemonGenerator.getInstance().addRandomBuff(p));
        }
    }
    ...
    public char goNorth() {
        ...
        try {
            loc.translate(-1, 0);
            charAtLoc = Map.getInstance().getCharAtLoc(loc);
    ...
    /**
     * Buffs all of Trainer's Pokemon.
     */
    public void buffAllPokemon() {
        for (int i = 0; i < this.pokemon.size(); i++) {
            this.pokemon.set(i, PokemonGenerator.getInstance().addRandomBuff(this.pokemon.get(i)));
        }
    }

    /**
     * Debuffs a single Pokemon based on the index
     *
     * @param index index of the Pokemon to be debuffed
     */
    public void debuffPokemon(int index) {
        this.pokemon.set(index, PokemonGenerator.getInstance().addRandomDebuff(this.pokemon.get(index)));
    }
    ...
    public String getPokemonList() {
        ...
        for (int i = 0; i < this.pokemon.size(); i++) {
            ...
            pokemonList += num + ". " + this.pokemon.get(i) + "\n";
        }
    ...
    public String toString() {
        ...
        s += "\nPokemon\n-------\n" + this.getPokemonList() + "\n";
        s += Map.getInstance().mapToString(this.loc);
